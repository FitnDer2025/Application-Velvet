...
test('web avatars are not covered and conversations open through the messages API', async () => {
  ...
  assert.match(worker, /velvet-beta-shell-v12/);
});

test('native capture protection obscures recording and notifies both parties', async () => {
  const [root, app, detail, endpoint] = await Promise.all([
  ...
  assert.match(detail, /screenshotProtection\.protect/);
  assert.match(detail, /VelvetPhotoWatermark/);
