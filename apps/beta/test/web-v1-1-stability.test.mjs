...
test('le menu mobile V1.1 a un contrôleur unique et reste au-dessus du voile', async () => {
  const [shell, messaging, styles] = await Promise.all([
  ...
  assert.match(messaging, /VelvetWebV11\?\.toggleMenu/);
...
test('le contrôleur mobile ouvre et referme réellement la feuille et ses états accessibles', async () => {
  ...
  assert.equal(window.VelvetWebV11.toggleMenu(), true);
...
test('le cache V23 force le chargement du correctif Web sur les PWA existantes', async () => {
  const [html, worker] = await Promise.all([
    read('apps/web/velvet-members-beta-live.html'),
    read('apps/beta/static/sw.js')
  ]);
  for (const asset of [
    'members-live.js?v=20260803-2',
    'velvet-messaging-upgrade.js?v=20260803-2',
    'velvet-social-realtime.js?v=20260803-2',
    'velvet-web-ios-parity.js?v=20260803-2',
    'pwa-ios.js?v=20260803-2'
  ]) {
