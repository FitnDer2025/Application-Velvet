(() => {
  ...
  function pilotView() {
    ...
    return `<section class="control-page" data-page="pilot">
      ${pageHead('Briefing en temps réel', 'Pilotage Zwit', 'Voici ce que Zwit a fait et ce qui mérite ton attention maintenant.')}
  ...
  function bind() {
    ...
    root.querySelectorAll('[data-subscription-status]').forEach((select) => select.addEventListener('change', () => mutate({ action: 'subscription_status', venueId: select.dataset.subscriptionStatus, status: select.value }, 'Abonnement Zwit Pro synchronisé.')));
  ...
  async function load() {
    root.innerHTML = '<div class="control-loading"><span></span><p>Lecture sécurisée des données Zwit…</p></div>';
    try { data = await api(); render(); }
    catch (error) { root.innerHTML = `<section class="control-page"><div class="control-empty"><div><strong>Zwit Contrôle est indisponible</strong><span>${safe(error.message)}</span><br><button class="control-btn secondary" type="button" data-retry>Réessayer</button></div></div></section>`; root.querySelector('[data-retry]')?.addEventListener('click', load); }
  }
