...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const [profiles, establishments, events, conversations, recommendations] = await Promise.all([
      ...
      restJson(env, '/rest/v1/establishments?select=id,slug,name,kind,description,city,address_public,phone_public,email_public,opening_hours,amenities,verified_at&visibility=eq.published&order=name.asc&limit=100', token),
